...
# 局域网访问启动脚本 - 8080端口版本（无需sudo）

echo "🌐 销售模拟器 - 局域网访问配置 (8080端口)"
echo "=========================================="
echo ""
...
NEXT_PUBLIC_SECONDME_REDIRECT_URI=http://$LOCAL_IP:8080/auth/callback

# API配置
NEXT_PUBLIC_API_URL=http://$LOCAL_IP:8080
...
SECONDME_REDIRECT_URI=http://$LOCAL_IP:8080/auth/callback

# CORS配置
CORS_ORIGIN=http://$LOCAL_IP:8080
EOF
...
# 启动nginx（无需sudo）
echo "启动nginx反向代理..."
nginx -c "$(pwd)/nginx-8080.conf" -t && nginx -c "$(pwd)/nginx-8080.conf"
...
echo "💡 其他设备访问："
echo "   1. 确保设备在同一局域网"
echo "   2. 在浏览器打开: http://$LOCAL_IP:8080"
echo ""
echo "🛑 停止服务："
echo "   nginx -s stop"
echo ""
